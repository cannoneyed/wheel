...
- Unit tests cover labeling, descriptions, status, controlled and uncontrolled values, densities, dividers, disabled, and read-only state.
- Browser tests cover repeated row selection, keyboard activation, group naming, status relationships, and parent mixed state.
- Catalog fixtures show every density, dividers, validation tones, disabled, read-only, horizontal, and parent patterns.
