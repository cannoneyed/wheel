...
/**
 * Solid port of upstream's `NumberFieldInput.test.tsx`. `describeConformance` is upstream's
 * shared React-only conformance harness with no Solid equivalent, so only behavioral tests carry
 * over. React `useState`/`setProps`-backed "Controlled" helper components become local
 * `createSignal`s read directly in JSX. The dev-warning message text is adjusted from upstream's
 * `Base UI: ...` to this repo's `warn()` convention, `Base UI Solid: ...` (see
  ...
  it('warns in development when clipboard text cannot be read during paste handling', () => {
    ...
    try {
      ...
      expect(warnSpy.mock.calls[0]?.[0]).toContain(
        'Base UI Solid: <NumberField.Input> could not read clipboard text during paste handling.',
      );
    } finally {
      warnSpy.mockRestore();
    }
