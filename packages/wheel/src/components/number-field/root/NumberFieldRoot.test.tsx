...
/**
 * Solid port of upstream's `NumberFieldRoot.test.tsx`. `describeConformance` is upstream's shared
 * React-only conformance harness with no Solid equivalent, so only behavioral tests carry over.
 * React `useState`-backed "App"/"Controlled" helper components become local `createSignal`s read
 * directly in JSX; MUI's `rerender`/`setProps` become updates to those same signals.
 *
 * Deviation: every upstream `fireEvent.change(input, ...)` against the *visible* text input
 * becomes `fireEvent.input(...)` here. Solid's `onChange` JSX prop binds to the native `change`
 * event (fires only on commit/blur); React's `onChange` is backed by the native `input` event
 * (fires every keystroke). `NumberFieldInput` binds to `onInput` to match upstream's per-keystroke
 * semantics (see the deviation note in `NumberFieldInput.tsx`), so tests simulating typing must
 * dispatch `input`, not `change`. The *hidden* native `<input type="number">` (autofill carrier)
 * intentionally keeps `onChange` — `fireEvent.change` calls against it are left unchanged.
 */
...
function NumberField(props: NumberFieldBase.Root.Props): JSX.Element {
  return (
...
describe('<NumberField />', () => {
  ...
  describe('typing behavior (parseable changes)', () => {
    ...
    it('accepts multi-character currency symbols while typing (e.g. pt-BR BRL)', () => {
      ...
      fireEvent.input(input, { target: { value: `${formatted}7` } });
    ...
    it('accepts multi-character unit symbols while typing (e.g. km/h)', () => {
      ...
      fireEvent.input(input, { target: { value: '1 km/h' } });
      fireEvent.input(input, { target: { value: '12 km/h' } });
    ...
    it('accepts exponent separators while typing (scientific notation)', () => {
      ...
      fireEvent.input(input, { target: { value: '1.5E3' } });
    ...
    it('ignores bidi/format control characters in the value (e.g. RTL exponent signs)', () => {
      ...
      fireEvent.input(input, { target: { value: '5E‎-1' } });
  ...
  describe('prop: allowWheelScrub', () => {
    it('does not scrub on wheel when disabled', () => {
      const onValueChange = vi.fn();
      render(() => (
    ...
    it('does not scrub on wheel when readOnly', () => {
      const onValueChange = vi.fn();
      render(() => (
    ...
    it('does not scrub on wheel while pinch-zooming (ctrlKey)', () => {
      const onValueChange = vi.fn();
      render(() => (
    ...
    it('does not scrub on wheel when the input is not focused', () => {
      const onValueChange = vi.fn();
      render(() => (
        <NumberField defaultValue={5} allowWheelScrub onValueChange={onValueChange} />
      ));
    ...
    it('uses largeStep when shift is held during wheel', () => {
      const onValueChange = vi.fn();
      render(() => (
        <NumberField
          defaultValue={0}
    ...
    it('calls onValueChange and onValueCommitted on wheel', () => {
      const onValueChange = vi.fn();
      const onValueCommitted = vi.fn();
      render(() => (
        <NumberField
    ...
    it('syncs the visible input value when using the mouse wheel after pasting', () => {
      const onValueChange = vi.fn();
  ...
  describe('Form', () => {
    it('should include the input value in the form submission', () => {
      let fieldValue = '';
    ...
    it('should not include formatting in the submitted value', () => {
      const format: Intl.NumberFormatOptions = {
        style: 'currency',
        currency: 'EUR',
        minimumFractionDigits: 2,
    ...
    it.skipIf(isJSDOM)('submits to an external form when `form` is provided', () => {
      let fieldValue = '';
    ...
    it('validates the parsed number when handling browser autofill', () => {
    ...
    it.each([
      ...
      ({ lockState, withField }) => {
        const onValueChange = vi.fn();
        // Solid JSX creates real DOM/reactive nodes the moment it's evaluated (unlike React's
        // lazily-instantiated virtual elements), so this can't be hoisted into a shared `const`
        // read from both ternary branches — evaluating it once, outside of either branch's
        // context, would resolve `useFieldRootContext()`/`useFormContext()` against whatever
        // ambient (default) context is active at that point rather than the one each branch
        // actually renders into. A small component function, invoked from inside each branch,
        // keeps the JSX creation lazy and scoped correctly.
  ...
  describe('Field', () => {
    ...
    it('is validated with latest value when validationMode=onBlur', () => {
    ...
    it('is validated with clamped value when validationMode=onBlur', () => {
    ...
    it('revalidates an external change after a blur that normalizes back to the current value', async () => {
      ...
      // unchanged `createValueChanged` won't fire to reset the flag.
      fireEvent.focus(input);
  ...
  describe('pasting at the caret', () => {
    ...
    it('keeps the caret just after the pasted text', () => {
      render(() => <NumberField defaultValue={123} />);
      const input = screen.getByRole<HTMLInputElement>('textbox');
