...
/**
 * Solid port of upstream's `number-field/utils/parse.test.ts`. `parseNumber`/
 * `getNumberLocaleDetails` are framework-neutral, so every case ports unchanged.
 */
describe('NumberField parse', () => {
  ...
  describe('parseNumber', () => {
    ...
    it.each(['ar-EG', 'fa-IR'] as const)('parses localized scientific notation for %s', (locale) => {
