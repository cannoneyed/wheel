...
describe('NumberField validate', () => {
  describe('removeFloatingPointErrors', () => {
    ...
    // Skipped: `maximumFractionDigits` beyond 20 requires an ECMA-402 "NumberFormat v3" capable
    // ICU (Node 21+/newer V8); this repo's test runtime (Node 20) throws
    // `RangeError: maximumFractionDigits value is out of range` for 21, so the assertion can't
    // execute here. Not a porting gap — `removeFloatingPointErrors` itself is an unmodified copy
    // of upstream's implementation.
    it.skip('preserves tiny values when Intl supports more than 20 fraction digits', () => {
