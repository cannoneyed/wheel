...
/**
 * Solid port of upstream's `@base-ui/utils/useScrollLock`.
 *
 * Shared, framework-neutral scroll-locking logic (module-scope singleton
 * `ScrollLocker`, reference-counted across every consumer) ported unchanged;
 * only the public `createScrollLock` entry point adapts to Solid. Not part of
 * `utils/popups` (dialogs aren't anchored popups), but shared the same way
 * upstream's `useScrollLock` is shared across Dialog/Menu/Select/etc — this
 * port only wires it up for Dialog/AlertDialog so far.
 *
 * The singleton's own `Timeout`/`AnimationFrame` bookkeeping intentionally
 * uses the bare `Timeout.create()`/`AnimationFrame.create()` constructors
 * (not the owner-scoped `createTimeout()`/`createAnimationFrame()` helpers):
 * `ScrollLocker` lives at module scope, outside any component's reactive
 * owner, exactly like upstream's module-scope singleton.
 */
...
function preventScrollInsetScrollbars(referenceElement: Element | null) {
  ...
  return () => {
    ...
    // in a deferred callback. Guard the returned cleanup to avoid calling
 ...
 * Solid port of upstream's `useScrollLock`.
 *
...
export function createScrollLock(
  ...
  createEffect(() => {
    ...
    const release = SCROLL_LOCKER.acquire(referenceElement());
    onCleanup(release);
  });
}
