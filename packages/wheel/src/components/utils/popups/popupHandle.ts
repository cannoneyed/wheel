...
import { warnEveryTime } from '../../base-utils/warn';
import {
 ...
 *
 * Detached triggers read `store` at setup and subscribe to be notified when the handle switches
...
> implements PopupHandleStoreProvider<HandleStore>
{
  ...
  /**
   * Subscribes to changes of the attached store pointer so detached triggers re-bind when a root
   * attaches or detaches. Returns a function that removes the listener.
   * @internal
   */
  ...
  /**
   * Points the handle at a root's store and notifies subscribers so detached triggers re-bind and
   * re-register into it (their registration re-fires on the store-pointer change). Returns a
   ...
   * detached triggers re-bind and migrate their registration to the new store.
   */
  ...
  protected openByTrigger(triggerId: string | null | undefined) {
    ...
    // Registered triggers normally live in the active root's store. At the moment a root attaches,
    // a still-mounted detached trigger has not re-registered into that store yet (it migrates when
    // its subscription fires): it is still registered wherever it lived before — the fallback
    ...
    // (newest first) and the fallback map so an imperative open-by-id called in that window still
    // resolves the trigger instead of treating it as missing.
