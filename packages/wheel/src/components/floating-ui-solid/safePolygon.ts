 ...
 *
 * Solid port: the geometry/close-handler-factory logic below is
 * framework-neutral and unchanged from upstream. The only adaptation is that
 * `context.elements.domReference`/`.floating` are `Accessor`s here (per the
 * Solid `ExtendedElements` shape), so they're invoked (`elements.domReference()`)
 * rather than read as plain properties.
 */
