...
/**
 * Solid port of upstream's `floating-ui-react/utils/tabbable.ts`. These are
 * plain DOM/array algorithms with no React dependency, so the port is
 * essentially verbatim aside from swapping the `React.FocusEvent` type in
 * `isOutsideEvent` for the native `FocusEvent`.
 */
