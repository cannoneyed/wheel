...
/**
 * Solid port of upstream's `floating-ui-react/utils/composite.ts`.
 *
 * Translation notes:
 * - Upstream's `listRef: React.RefObject<Array<HTMLElement | null>>` becomes a
 *   plain `{ current: Array<HTMLElement | null> }` object (locked convention)
 *   — these are mutable value containers, not reactive state.
 * - `React.KeyboardEvent` becomes the native `KeyboardEvent` (no synthetic
 *   event layer in Solid).
 */

export type DisabledIndices = ReadonlyArray<number> | ((index: number) => boolean);

/**
 * Resolves a `MaybeAccessor<DisabledIndices | undefined>` option.
 *
 * `DisabledIndices` can itself be a function (an index predicate, arity 1),
 * so the generic `access()` helper (which calls *any* function value) can't
 * be used here — it would invoke a predicate with zero arguments instead of
 * treating it as the plain value. Disambiguate by arity instead, matching
 * this codebase's established thunk-vs-getter convention: a zero-arg
 * function is the `MaybeAccessor` wrapper (call it), anything else (a
 * one-arg predicate, an array, or `undefined`) is the plain value.
 */
export function accessDisabledIndices(
  value: DisabledIndices | undefined | (() => DisabledIndices | undefined),
): DisabledIndices | undefined {
  if (typeof value === 'function' && value.length === 0) {
    return (value as () => DisabledIndices | undefined)();
  }
  return value as DisabledIndices | undefined;
}
  ...
  sizes.forEach(({ width, height }, index) => {
    if (width > cols) {
      if (process.env.NODE_ENV !== 'production') {
        throw new Error(
          `[Base UI Solid]: Invalid grid - item width at index ${index} is greater than grid columns`,
        );
      }
    }
...
export function getGridCellIndices(indices: (number | undefined)[], cellMap: (number | undefined)[]) {
