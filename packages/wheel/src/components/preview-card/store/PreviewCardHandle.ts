...
> {
  ...
  /**
   * Whether the preview card is currently open. Returns `false` while no root is attached to the
   * handle.
   */
  get isOpen() {
...
/**
 * Creates a new handle to connect a `PreviewCard.Root` with detached `PreviewCard.Trigger`
 * components.
 */
