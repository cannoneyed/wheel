...
/**
 * Value/label resolution helpers shared by `Select.Value` and the select store.
 * Solid port of upstream's `internals/resolveValueLabel.tsx`.
 */
...
export function hasNullItemLabel(items: ItemsInput): boolean {
  ...
  if (isGroupedItems(arrayItems)) {
    for (const group of arrayItems) {
      for (const item of group.items as ReadonlyArray<LabeledItem>) {
...
export function resolveSelectedLabel(
  ...
  if (Array.isArray(items)) {
    ...
    const flatItems: ReadonlyArray<LabeledItem> = isGroupedItems(arrayItems)
      ? arrayItems.flatMap((group) => group.items as ReadonlyArray<LabeledItem>)
      : arrayItems;
...
export function resolveMultipleLabels(
  ...
  values.forEach((value, index) => {
    ...
    nodes.push(resolveSelectedLabel(value, items, itemToStringLabel));
  });
