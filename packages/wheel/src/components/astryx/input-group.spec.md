# InputGroup behavior specification

- Connects an input with prefixes, suffixes, and actions.
- Astryx family coverage: `Field`, `FieldLabel`, `FieldStatus`; success, warning, error; attached/detached. `InputGroup`, `InputGroupText`; sizes `sm`–`lg`
- Wheel family contract: Extend Wheel Field. Add Input Group as a standalone composed entry. Keep labels and status as Field parts.
