...
- Sizes and colors one application icon without changing its source.
- Astryx family coverage: `Icon`; sizes `xsm`–`lg`; primary, secondary, tertiary, disabled, accent, success, error, warning, inherit
- Wheel family contract: Add a renderer and icon registry contract. Keep icon data separate from component code so applications can tree-shake sets.
