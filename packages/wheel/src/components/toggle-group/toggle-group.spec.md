...
- Unit tests cover single and multiple state, controlled and uncontrolled values, deselection, cancellation, naming, inheritance, disabled members, both orientations, both layouts, focus looping, right-to-left direction, and Toolbar composition.
- Browser tests cover pointer selection, keyboard selection, focus movement, single deselection, multiple selection, fill layout, and vertical orientation.
- Catalog fixtures show single, multiple, horizontal, vertical, hug, fill, all sizes, all variants, and disabled states.
