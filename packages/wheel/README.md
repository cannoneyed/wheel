...
import { collection, query, mutation } from 'wheel/sync';
...
Import `wheel/components/styles` once to apply the default Mira zinc theme. Each component family also has a deep entry such as `wheel/components/dialog`. Override `--wheel-component-*` tokens or the stable `wheel-*` part classes to customize the theme.
